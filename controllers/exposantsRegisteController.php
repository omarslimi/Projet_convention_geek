...
if(count($_POST) > 0){
    ...
    if(!empty($_POST['user_NameExposants'])){
        if(preg_match($regex['lastName'],$_POST['user_NameExposants'])){
            $user_NameExposants = strip_tags($_POST['user_NameExposants']);
        } else {
            $formErrors['user_NameExposants'] = 'Veuillez entrer un nom correct valide';
        }
    } else {
        $formErrors['user_NameExposants'] = 'Veuillez entrer un nom';
    }
} 
